...
ggplot(data = rpls_aggrege_large %>%
  ...
  geom_point(aes(
    x = Parc_de_moins_de_5_ans_pourcent,
...
> A partir des données de la table `sdg_indicators`, reprendre le nuage de point obtenu à partir des données ODD : taux de mortalité maternelle (Maternal_mortality_ratio) en fonction du produit intérieur brut (Gross_Domestic_Product_GDP) (Chapitre 4.1)
> 
...
```{r eval=T,fig.height=8,fig.width=12,message=F,warning=F,echo=FALSE}
rpls_aggrege %>%
  filter(
    TypeZone == "Régions",
    Indicateur %in% c(
      "3 et 4 pièces_pourcent",
