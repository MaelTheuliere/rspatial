...
La terre est un objet particulier, qu'on a pensé plate, puis cylindrique, puis ronde, puis plate, puis ronde, puis ellipsoïde. Maintenant, on sait que la terre, elle a cette forme là : 
...
Les systèmes de coordonnées de référence va permettre de se repérer sur la surface de la terre.
...
ce *géoïde* est proche d'un ellipsoïde, c'est un dire une surface obtenue en faisant tournée une ellipse sur son axe. La terre est en effet compressée :  le rayon à l'équateur est 11,5 km plus long que le rayon polaire (Maling 1992).
...
On va distinguer deux type de datum : les datum *locaux* et les datum *globaux*.
...
Dans un systèmes de coordonnées géographique, les lieux sont donc identifiés par des *angles* et non des *mètres*.
...
En général un système de coordonnées projeté ne permet de préserver qu'une ou deux de ces propriétés. On classifie d'ailleurs ces projections selon les propriétés conservées  : 
